...
from LazyDeveloperr import app, config, db, logger, queue, yt, userbot
from LazyDeveloperr.music_helpers import utils
    ...
    async def wrapper(_, m: types.Message, *args, **kwargs):
        ...
        if not userbot.clients:
            return await m.reply_text(
                "⚠️ **Assistant Userbot is not configured!**\n\n"
                "Telegram Voice Calls require a Pyrogram Userbot String Session (`SESSION` or `SESSION1` in your `.env`). "
                "Telegram API does not allow bot accounts to join voice chats directly."
            )
        ...
        m3u8 = kwargs.get("m3u8") or (url and not yt.valid(url) and not spotify.valid(url))
                    ...
                    try:
                        ...
                        return await m.reply_text(
                            m.lang["play_banned"].format(
                                ...
                                client.id,
                                client.mention,
                                f"@{getattr(client, 'username', '')}" if getattr(client, 'username', None) else None,
                            )
                        )
