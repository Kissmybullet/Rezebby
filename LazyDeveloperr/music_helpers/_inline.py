...
from LazyDeveloperr import app, config, lang
from LazyDeveloperr.music_core.lang import lang_codes
...
class Inline:
    ...
    def start_key(
        ...
        bot_uname = getattr(app, "username", None) or (app.me.username if getattr(app, "me", None) else None)
        if not bot_uname:
            try:
                from LazyDeveloperr import BOT_USERNAME
                bot_uname = BOT_USERNAME
            except Exception:
                bot_uname = "bot"
        sup_url = config.SUPPORT_CHAT or "worldwide_friend_zone"
        if not str(sup_url).startswith("http"):
            sup_url = f"https://t.me/{sup_url}"

        chan_url = config.SUPPORT_CHANNEL or sup_url
        if not str(chan_url).startswith("http"):
            chan_url = f"https://t.me/{chan_url}"

        rows = [
            [
                self.ikb(
                    text=lang["add_me"],
                    url=f"https://t.me/{bot_uname}?startgroup=true",
                )
            ],
            [self.ikb(text=lang["help"], callback_data="help")],
            [
                self.ikb(text=lang["support"], url=sup_url),
                self.ikb(text=lang["channel"], url=chan_url),
            ],
        ]
        if private:
            rows += [
