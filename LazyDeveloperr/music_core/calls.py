...
from LazyDeveloperr import app, config, db, lang, logger, queue, thumb, userbot, yt
from LazyDeveloperr.music_helpers import Media, Track, buttons
...
class TgCall(PyTgCalls):
                ...
                if remaining <= 15:
                    ...
                    if next_media and not next_media.file_path:
                        next_media.file_path = await yt.download(
                            ...
                            artist=getattr(next_media, 'artist', None),
                        )
                    break
    ...
    async def play_media(
        ...
        abs_path = os.path.abspath(media.file_path) if media.file_path else None
        ...
        try:
            ...
            # Attempt to auto-start group call via userbot if possible
            ub_client = await db.get_client(chat_id)
            started = False
            if ub_client:
                try:
                    from pyrogram.raw import functions
                    import random
                    await ub_client.invoke(
                        functions.phone.CreateGroupCall(
                            peer=await ub_client.resolve_peer(chat_id),
                            random_id=random.randint(10000, 9999999),
                        )
                    )
                    started = True
                    ...
                    logger.error(f"Failed to auto-start group call: {ex_start}")
            ...
            logger.error(f"Unhandled exception in play_media for {chat_id}: {ex}")
            await self.stop(chat_id)
            await message.edit_text(
                f"⚠️ **Streaming Error:** `{type(ex).__name__}`: {ex}\n\n"
                "Please make sure:\n"
                "1. A Video Chat is currently active in this group.\n"
                "2. The Assistant userbot is in this group and has Admin permissions."
            )
        finally:
            await asyncio.sleep(5)
            self.restarting[chat_id] -= 1
                    ...
                    if media:
                        ...
                        if not media.file_path:
                            media.file_path = await yt.download(
                                media.id, video=media.video, title=media.title,
    ...
    async def boot(self) -> None:
        ...
        for ub in userbot.clients:
            ...
            client.username = getattr(ub, "username", "") or ""
            client.id = getattr(ub, "id", None)
            client.name = getattr(ub, "name", "")
            client.mention = getattr(ub, "mention", "")
            await client.start()
