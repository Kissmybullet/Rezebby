...
from LazyDeveloperr import dispatcher as n
from LazyDeveloperr.modules.disable import DisableAbleCommandHandler, DisableAbleMessageHandler
...
class MukeshTelegramHandler:
    ...
    def message(
        ...
        def _message(func):
            try:
                ...
                LOGGER.debug(
                    f"[LazyDeveloperr-Msg] Loaded filter pattern {pattern} for function {func.__name__}"
                )
    ...
    def callbackquery(self, pattern: str = None, run_async: bool = True):
        def _callbackquery(func):
            ...
            LOGGER.debug(
                f"[LazyDeveloperr-Callback] Loaded callbackquery handler with pattern {pattern} for function {func.__name__}"
            )
            return func
