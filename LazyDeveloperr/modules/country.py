...
from LazyDeveloperr import telethn as borg
from LazyDeveloperr.events import register


@register(pattern=r"^/country(?:@\w+)?(?:\s+(.+))?")
async def msg(event):
    if event.fwd_from:
        return
    input_str = event.pattern_match.group(1)
    if not input_str:
        return await event.reply("Usage: `/country <country name>`")
    lol = input_str.strip()
    country = CountryInfo(lol)
    try:
        a = country.info()
    except Exception:
        return await event.reply("Country Not Available Currently")
