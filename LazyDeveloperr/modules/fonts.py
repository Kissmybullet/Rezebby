...
async def style_buttons(c, m, cb=False):
    ...
    buttons = [
        ...
        [
            ...
            InlineKeyboardButton("𝕲𝖔𝖙𝖍𝖎𝖠", callback_data="style+gothic_bolt"),
        ],
        [
...
async def style(c, m):
    ...
    if m.message.reply_to_message and m.message.reply_to_message.text:
        orig_text = m.message.reply_to_message.text
    elif m.message.text:
        orig_text = m.message.text.strip("`")
    if " " in orig_text:
        orig_text = orig_text.split(" ", 1)[1]
    new_text = cls(orig_text) if orig_text else "No text"
    try:
