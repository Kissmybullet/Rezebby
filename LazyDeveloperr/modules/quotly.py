...
class Quotly:
    ...
    async def create_quotly(
        ...
        for api_url in apis:
            try:
                request = await async_searcher(api_url, post=True, json=content, re_json=True, timeout=10)
                if isinstance(request, dict) and request.get("ok"):
                    break
            except Exception:
                continue

        if request and isinstance(request, dict) and request.get("ok"):
            with open(file_name, "wb") as file:
                image = base64.decodebytes(request["result"]["image"].encode("utf-8"))
                file.write(image)
            return file_name
        raise Exception("Quote generation API is currently offline. Please try again later.")
...
@register(pattern=r"^/q(?:@\w+)?(?:\s+(.*))?")
async def quott_(event):
    match = (event.pattern_match.group(1) or "").strip()
    if not event.is_reply:
        return await event.reply("ᴘʟᴇᴀsᴇ ʀᴇᴘʟʏ ᴛᴏ ᴀ ᴍᴇssᴀɢᴇ")
