...
import LazyDeveloperr.modules.sql.chatbot_sql as sql
from LazyDeveloperr import BOT_ID, BOT_NAME, BOT_USERNAME, dispatcher
from LazyDeveloperr.modules.helper_funcs.chat_status import user_admin, user_admin_no_reply
from LazyDeveloperr.modules.log_channel import gloggable
...
def chatbot(update: Update, context: CallbackContext):
    ...
    if message.text and not message.document:
        ...
        try:
            res = api.chatgpt(message.text)
            reply = res.get("results") or res.get("result") if isinstance(res, dict) else str(res)
            if reply:
