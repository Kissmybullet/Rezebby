...
from LazyDeveloperr import telethn,pbot
from LazyDeveloperr.modules.helper_funcs.telethn.chatstatus import (
    can_delete_messages,
    user_is_admin,
)
...
PURGE_HANDLER = purge_messages, events.NewMessage(pattern=r"^[!/]purge(?:@\w+)?(?:\s+(.*))?$")
DEL_HANDLER = delete_messages, events.NewMessage(pattern=r"^[!/]del(?:@\w+)?$")
SPURGE_HANDLER = spurge_messages, events.NewMessage(pattern=r"^[!/]spurge(?:@\w+)?(?:\s+(.*))?$")
