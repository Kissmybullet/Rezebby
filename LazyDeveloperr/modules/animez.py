...
from LazyDeveloperr import dispatcher,OWNER_ID
...
def upcoming(update: Update, context: CallbackContext):
    ...
    """
    try:
        response = requests.post("https://graphql.anilist.co", json={"query": query}, timeout=10).json()
        media = response["data"]["Page"]["media"]
        upcoming_message = "<b>Top Upcoming Anime:</b>\n\n"
        for idx, item in enumerate(media, 1):
            title = item["title"]["english"] or item["title"]["romaji"]
            url = item["siteUrl"]
            upcoming_message += f"{idx}. <a href='{url}'>{html.escape(title)}</a>\n"
        update.effective_message.reply_text(upcoming_message, parse_mode=ParseMode.HTML, disable_web_page_preview=True)
    except Exception as e:
        update.effective_message.reply_text(f"Failed to fetch upcoming anime: {e}")
